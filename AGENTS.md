...
replaced — by a knowledge graph in [FalkorDB](https://www.falkordb.com/), built and queried with
the **official `falkordb` Python client directly** (raw Cypher). Not via LangChain:
`langchain_community.graphs.FalkorDBGraph` / `GraphCypherQAChain` / `LLMGraphTransformer` need the
LLM to reliably emit structured tool-calls (to build the graph) or syntactically valid Cypher (to
query it) — free OpenRouter models do this inconsistently in practice, and both
`langchain-community` and `langchain-experimental` are upstream-deprecated ("sunset") packages.
This mirrors the direct-client pattern used by
[`CodeGraphContext`](https://github.com/CodeGraphContext/CodeGraphContext) for its own FalkorDB
backend.
...
2. **Build the graph** (offline, run once / re-run after literature changes): `python scripts/build_flu_graph.py`
   ...
   - Per chunk, calls `MLAgentBench.LLM.complete_text()` (the same OpenRouter-routed, retrying LLM
     call used everywhere else in this project) with a prompt asking for entities/relationships as
     JSON; parses and validates the JSON against a fixed whitelist before writing
   ...
   - Free models are unreliable here (invalid JSON, or just don't support function calling well
     enough for clean structured output) — `--model openai/gpt-4o-mini` (default, set via
     `GRAPH_BUILD_MODEL` in `.env`) is cheap and far more consistent for this one-time offline step
...
3. **Query (hybrid)**: `search_flu_context_rag(query, k=5)` in `MLAgentBench/agents/agent_specialized.py`
   ...
   - `graph_context` — keyword-matched Cypher: extracts query tokens, matches against node names
     (case-insensitive substring), returns the 1-hop neighborhood of matches. No LLM call needed
     for retrieval at all — deterministic, free, instant.
   - Degrades to vector-only (empty `graph_context`) if FalkorDB is unreachable — never raises
...
`FALKORDB_HOST`, `FALKORDB_PORT`, `FALKORDB_GRAPH_NAME`, `GRAPH_BUILD_MODEL` in `.env` (see `.env.example`). The Literature RAG (PixelRAG, visual) is untouched by this — it has no extracted text/entities to build a graph from today.
