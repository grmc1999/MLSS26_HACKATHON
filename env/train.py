...
class ConditionalDenoiser(nn.Module):
    ...
    def __init__(self, forecast_steps=FORECAST_STEPS, input_steps=INPUT_STEPS, hidden_dim=64, time_dim=64, dropout=0.15):
        super().__init__()
        self.forecast_steps = forecast_steps
...
def diffusion_loss(model, x0, c, device):
    ...
    snr = a_bar / (1 - a_bar + 1e-6)
    loss_weight = torch.clamp(1.0 / snr, max=5.0)
    l_diff = (loss_weight * F.mse_loss(eps_hat, noise, reduction='none')).mean()
