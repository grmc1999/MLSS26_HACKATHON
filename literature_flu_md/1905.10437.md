...
in Fig. 1 for brevity). The ℓ-th block accepts its respective input xℓand outputs two vectors, bxℓand
byℓ. For the very ﬁrst block in the model, its respective xℓis the overall model input — a history
...
nonlinearities. It predicts basis expansion coefﬁcients both forward, θ f , (forecast) and backward, θ b,
(backcast). Blocks are organized into stacks using doubly residual stacking principle. A stack may
have layers with shared gb and g f . Forecasts are aggregated in hierarchical fashion. This enables
...
2H to 7H. For the rest of the blocks, their inputs xℓare residual outputs of the previous blocks. Each
block has two outputs: byℓ, the block’s forward forecast of length H; and bxℓ, the block’s best estimate
...
ℓand the backward θ b
ℓpredictors of expansion coefﬁcients (again, note
that the block index ℓis dropped for θ b
ℓ, θ f
ℓ, gb
ℓ, gf
...
the backward gb
ℓand the forward gf
ℓbasis layers that accept the respective forward θ f
ℓand backward
θ b
ℓexpansion coefﬁcients, project them internally on the set of basis functions and produce the
backcast bxℓand the forecast outputs byℓdeﬁned in the previous paragraph.
...
ℓ= W f
ℓhℓ,4. The FC layer is a standard
...
forecast byℓby properly mixing the basis vectors supplied by gf
ℓ. Additionally, this sub-network
predicts backward expansion coefﬁcients θ b
ℓused by gb
...
outputs can be represented adequately via varying expansion coefﬁcients θ f
ℓand θ b
ℓ. As shown below,
gb
ℓand gf
...
sequential analysis of the input signal. Previous block removes the portion of the signal bxℓ−1 that
...
forecast byℓthat is ﬁrst aggregated at the stack level and then at the overall network level, providing a
hierarchical decomposition. The ﬁnal forecast by is the sum of all partial forecasts. In a generic model
...
The generic architecture does not rely on TS-speciﬁc knowledge. We set gb
ℓand gf
ℓto be a linear
...
the predictive decomposition of the partial forecast byℓin the basis V f
ℓlearned by the network. Matrix
V f
ℓhas dimensionality H ×dim(θ f
ℓ). Therefore, the ﬁrst dimension of V f
...
are imposed on the form of V f
ℓ, the waveforms learned by the deep model do not have inherent
structure (and none is apparent in our experiments). This leads to byℓnot being interpretable.
...
omitted for clarity. Now we will consider both stack level and block level indexing. For example, bys,ℓ
...
s,ℓ,
where θ f
s,ℓare polynomial coefﬁcients predicted by a FC network of layer ℓof stack s described by
equations (1); and T = [1,t,...,tp] is the matrix of powers of t. If p is low, e.g. 2 or 3, it forces bytr
s,ℓ
to mimic trend.
...
s,ℓ,
where θ f
s,ℓare Fourier coefﬁcients predicted by a FC network of layer ℓof stack s described by
equations (1); and S = [1,cos(2πt),...cos(2π⌊H/2−1⌋t)),sin(2πt),...,sin(2π⌊H/2−1⌋t))] is the
matrix of sinusoidal waveforms. The forecast byseas
s,ℓ
is then a periodic function mimicking typical
seasonal patterns.
...
the expansion coefﬁcients θ f that basis g f takes as inputs. The inner learning proceeds through a
...
the expansion coefﬁcients θ f which eventually feed into g f in each block (which get added together
...
to the next block by producing backward expansion coefﬁcients θ b, thus conditioning the learning
...
also include the V’s which deﬁne the g f non-parametrically. This point of view is further reinforced
...
master/Dataset.
M4 Team.
M4 competitor’s guide: prizes and rules, 2018b.
URL www.m4.unic.ac.cy/
...
4.682
11.986
Best ML/TS combination
13.528
9.733
12.639
4.118
11.720
DL/TS hybrid, M4 winner
13.176
9.679
12.126
4.014
...
4.41
13.01
DOTM (Fiorucci et al., 2016)
15.94
9.28
13.74
4.58
12.90
EXP (Spiliotis et al., 2019)
16.39
8.98
13.43
5.46
...
4.26
n/a
BaggedETS.BC (Bergmeir et al., 2016)
17.49
9.89
13.74
n/a
...
Published as a conference paper at ICLR 2020
C.4
DETAILED RESULTS: ELECTRICITY AND TRAFFIC DATASETS
...
Published as a conference paper at ICLR 2020
C.5
DETAILED RESULTS: COMPARE TO DEEPAR, DEEP STATE SPACE MODELS
...
and the Seasonality stacks of the interpretable model, respectively; FORECAST-I is their summation.
E
DETAILED SIGNAL TRACES OF INTERPRETABLE INPUTS PRESENTED IN
FIGURE 2
