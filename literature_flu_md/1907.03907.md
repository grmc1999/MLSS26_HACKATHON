...
arXiv:1907.03907v1  [cs.LG]  8 Jul 2019
...
A simple way to handle irregularly-timed samples is to include the
...
Neural ODEs [Chen et al., 2018] are a family of
continuous-time models which deﬁne a hidden state h(t) to be the solution to an ODE initial-value
problem (IVP):
dh(t)
dt
= fθ(h(t), t)
...
(4)
Chen et al. [2018] showed how adjoint sensitivities can be used to compute memory-efﬁcient gradients
...
standard RNN update hi = RNNCell(h′
...
Autoregressive Modeling with the ODE-RNN
The ODE-RNN can straightforwardly be used
...
i.e. they factor the joint density p(x) = Q
...
the pre-activations h′ evolve according to an ODE between observations, instead of being ﬁxed.
Input: Data points and their timestamps {(xi, ti)}i=1..N
h0 = 0
for i in 1, 2, ..., N do
h′
i = ODESolve(fθ, hi−1, (ti−1, ti))
▷Solve ODE to get state at ti
hi = RNNCell(h′
i, xi)
▷Update hidden state given current observation xi
end for
...
3.4
Batching and computational complexity
...
Latent variable models versus autoregressive models
We refer to models which iteratively com-
pute the joint distribution p(x) = Q
...
Extrapolation
Next, we show that a time-invariant ODE can recover stationary periodic dynamics
...
Interpolation
The standard RNN and the ODE-RNN are straightforward to apply to the interpola-
...
Extrapolation
In the extrapolation setting, we use the standard RNN or ODE-RNN trained on the
...
Interpretability of the latent state
Figure 6 shows how the norm of the latent state time-derivative
...
standard deviations. We run one-sided t-test to establish a statistical signiﬁcance. Best models are
...
2.789 ± 0.771
2.208 ± 0.050
Table 6: Per-sequence classiﬁcation.
AUC on Physionet.
Method
AUC
RNN ∆t
...
11
Supplementary tables and ﬁgures
1
Experiment setup
We test the ODE models for two tasks: interpolation and extrapolation.
Interpolation
Consider time series with time points (t0...tN). In interpolation task we condition on the
...
experiments with diﬀerent proportion of subsampled points (ranging from 10% to 50%, results shown in
...
Extrapolation
In extrapolation task we split the time series into two parts: (t0...tN2) and (tN2..tN). We
encode the ﬁrst half of the time series and reconstruct the second half.
...
Autoencoder models are straightforward to use for extrapolation task. We run the encoder on the ﬁrst
half of the sequence and decode the second half. For autoregressive models, we train the model to perform
interpolation ﬁrst, and then perform extrapolation at test time by re-feeding previous predictions of the
...
Choice of ﬁrst time point of ODESolve
The generative model has a special time point t0 where we
...
time point of the ﬁrst observation, as shown in eq. (5-7), and run ODE-RNN backwards in time to obtain
...
Decoder
Interpolation
Backwards in time tN →t0
q(z(t0)|x0..xN)
Forward in time in [t0, tN] interval
Extrapolation
Forward in time t0 →tN/2
q(z(tN/2)|x0..xN/2)
Forward in time in [tN/2, tN] interval
1
arXiv:1907.03907v1  [cs.LG]  8 Jul 2019
2
Model details
...
0), gσ(z′
0)
▷gµ and gσ are feed-forward NN
z0 ∼N(µz0, σz0)
{zi} = ODESolve(f, z0, (t0...tN))
˜xi = OutputNN(zi) for all i = 1..N
Return: {˜xi}i=1..N
2.3
Modelling poisson process likelihood
To model poisson process on Physionet dataset, we augment generative ODE
d
dtz = f(z) by adding extra
...
R t
0 λ(τ)dτ is λ(t). Thus, the augmented ODE is deﬁned as follows:
d
dt


...
R t
0 λ(τ)dτ to zero (notice that
R 0
0 λ(τ)dτ = 0. Initial values of z and zλ are
sampled from approximate posterior. The augmented ODE is solved using a call to ODESolve.
Generative model with Poisson Process The joint generative model is speciﬁed as
p(z0)p(t0, . . . , tN|z0) QN
i=0 p(xi|z0), where the distributions are speciﬁed below.
p(z0) = Normal
 z0; 0, I

(1)
...
(4)
2
The Latent ODE framework speciﬁes a generative model for time series.
y0 ∼p(y0)
(5)
λ(t) = ODESolve(y0, fλ, θfλ, t)
(6)
t1...tN ∼PoissonProcess(λ(t))
(7)
y1, ...yN = ODESolve(y0, fy, θfy, t1...tN)
(8)
xi ∼p(x|yi, θx), ∀i ∈{1..N}
(9)
3
...
Toy dataset
We generate 1000 one-dimentional trajectories with 100 time points in each on the [0, 5]
interval. We use sinusoid with ﬁxed amplitude of 1 and sample frequency from [0.5, 1] interval. We sample
the starting point from N(µ = 1, σ = 0.1).
MuJoCo
We generate 10,000 simulations of the ”Hopper” model from Deep Mind Control Suite and
...
Physionet
PhysioNet contains the data from the ﬁrst 48 hours of patients in ICU. We have excluded four
...
Each feature in each patients was measured at diﬀerent time points. In order to perform batching of the
...
Human Activity
The original dataset contains 25 sequences from ﬁve people (6600 points on average in
...
3
General notes
For all datasets, we take the union of all time points in the dataset and run all models
...
Architecture
ODE function
We use a feed-forward neural net for ODE function f. See section 5 for sizes of the network
in each experiment.
...
with the speciﬁed tolerance. For this reason, we do not recommend using ReLU.
ODE Solver
We used ODE Solvers from torchdiﬀeq python package. We used ﬁfth-order ”dopri5” solver
...
Loss
To compute data log-likelihood, We use negative gaussian log-likelihood with ﬁxed variance as a
reconstruction loss. We used ﬁxed variance of 0.001 for MuJoCo and 0.01 for other datasets. We report
MSE on the time series in the test set. For classiﬁcation task, we use cross-entropy loss. To compute ELBO
...
provided per time series, and reconstruction loss prevents overﬁtting on classiﬁcation task.
On Human
Activity dataset, the labels are provided per time point and training with CE loss only does not lead to
overﬁtting.
RNN Baselines
We follow the available implementation of RNN GRU-D: https://github.com/zhiyongc/
...
First, we choose the hyperparameters such that delivered the best perfor-
...
Hyperparameters for Latent ODE model
We empirically found that ODE of the same or slightly
...
model. For Human activity classiﬁcation task (12 data dimentions), we used 15 latent dimensions in gener-
ative model and 100 in recognition model.
Toy dataset
To model 1-dimensional toy dataset, we used 10 latent dimensions in the generative model
...
4
MuJoCo
In encoder-decoder models, we used 15 latent dimensions in generative model, 30 dimensions in
...
Physionet (interpolation and extrapolation)
In encoder-decoder models, we used 20 latent dimensions
...
Fitting Poisson process on Physionet dataset
For ﬁtting Poisson process along with the likelihood
...
Classiﬁcation on Physionet
In classiﬁcation task, the goal is to classify each patient with a binary label.
We used ”in-hospital mortality” label from the original dataset. To prevent overﬁtting, we train the model
together with reconstruction loss with coeﬃcient 100 on cross-entropy loss. We compute reconstruction loss
on all 8000 patients, and cross-entropy loss on 4000 labeled patients. We used a 2-layer classiﬁer with 300
...
activation.
Classiﬁcation on Human Activity
On Human Activity dataset, the task is to classify each time point
by the type of activity. We used a linear classiﬁer on each hidden state hi.
...
We use KL annealing for VAE models with coeﬃcient 0.99.
6.1
...
We used the ODE solvers from torchdiﬀeq package (https://github.com/rtqichen/torchdiffeq)
Datasets:
...
time series to (t0..ti) and computed number of ODE function evaluations for diﬀerent i. Each line shows a
...
are reconstructions for diﬀerent samples of z0 in Latent ODE model. (b) Corresponding latent state in the
recognition model (ﬁrst dimension). The recognition model encodes the data backwards in time (from right
...
with
poisson
Figure 6: True posterior p(z0|x1..xN) of Latent ODE trained on Physionet dataset with ﬁve features and
two-dimensional latent space with and without poisson process likelihood.
We train the model with 2-
dimensional latent space on a subset of ﬁrst ﬁve attributes. We compute the unnormalized density of the
...
75
Inferred rate
Figure 8: Examples of the inferred Poisson rate λ(t) (green line) for two selected features of diﬀerent patients
