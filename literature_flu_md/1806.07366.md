...
Memory efﬁciency
In Section 2, we show how to compute gradients of a scalar-valued loss with
...
Euler’s method is perhaps the simplest method for solving ODEs. There
...
Scalable and invertible normalizing ﬂows
An unexpected side-beneﬁt of continuous transforma-
...
Unlike recurrent neural networks, which require discretizing
...
Software
To solve ODE initial value problems numerically, we use the implicit Adams method
...
While det is not a linear function, the trace function
is, which implies tr(P
n Jn) = P
...
We can specify the parameters of a ﬂow as a function of t, making the
...
Density matching
We conﬁgure the CNF as described above, and train for 10,000 iterations
...
A useful property of continuous-time normalizing ﬂows is that
...
We can train this latent-variable model as a variational autoen-
...
The fact that an observation oc-
...
To generate irregular timestamps, we randomly sample
...
Figure 8c shows latent trajectories projected onto the ﬁrst two dimen-
...
Uniqueness
When do continuous dynamics have a unique solution? Picard’s existence theo-
...
Reconstructing the state trajectory by running the dynamics
...
Adaptive computation
One can adapt computation time by training secondary neural networks
...
Recent work developed reversible versions
...
Much recent work has proposed learning differential equations
...
The dolfin library (Farrell et al., 2013) implements adjoint
...
terfactual reasoning with continuous-time, continuous-valued interventions.
arXiv preprint
...
Special Cases
Planar CNF.
Let f(z) = uh(wz + b), then ∂f
∂z = u ∂h
∂z
T. Since the trace of an outer product is the inner
product, we have
∂log p(z)
∂t
= −tr
...
Hamiltonian CNF.
The continuous analog of NICE (Dinh et al., 2014) is a Hamiltonian ﬂow, which splits
...
A.2
Connection to Fokker-Planck and Liouville PDEs
...
Appendix B
A Modern Proof of the Adjoint Method
...
dt
= f(z(t), t, θ), where θ are the parameters. We will prove that if
...
∂z , −a(t)T ∂f
∂θ , −a(t)T ∂f
∂t ] ▷Compute vector-Jacobian products
[z(t0),
∂L
∂z(t0), ∂L
∂θ , ∂L
∂t0 ] = ODESolve(s0, aug_dynamics, t1, t0, θ)
▷Solve reverse-time ODE
return
∂L
∂z(t0), ∂L
...
f l a t t e n
from
autograd . b u i l t i n s
import
t u p l e
o d e i n t = p r i m i t i v e ( scipy . i n t e g r a t e . o d e i n t )
def
g r a d _ o d e i n t _ a l l ( yt ,
func ,
y0 ,
t ,
func_args ,
...
2017
#
h t t p s : / / a r x i v . org / pdf /1711.08079. pdf
T , D = np . shape ( yt )
f l a t _ a r g s ,
u n f l a t t e n
=
f l a t t e n ( func_args )
def
f l a t _ f u n c ( y ,
t ,
f l a t _ a r g s ) :
...
func ( y ,
t ,
∗u n f l a t t e n ( f l a t _ a r g s ) )
def
unpack ( x ) :
#
y ,
...
x [2 ∗D + 1 : ]
def
augmented_dynamics ( augmented_state ,
t ,
f l a t _ a r g s ) :
# Orginal
system
...
vjp_y ,
_ ,
_ = unpack ( augmented_state )
v j p _ a l l ,
dy_dt = make_vjp ( f l a t _ f u n c ,
argnum =(0 ,
1 ,
2 ) ) ( y ,
t ,
...
vjp_y ,
vjp_t ,
vj p _a r g s = v j p _ a l l (−vjp_y )
r e t u r n
np . hstack ( ( dy_dt ,
vjp_y ,
vjp_t ,
v j p _ a r g s ) )
def
v j p _ a l l ( g ,∗∗kwargs ) :
vjp_y = g[−1,
: ]
vjp_t0 = 0
t i m e _ v j p _ l i s t = [ ]
v jp _ ar g s = np . zeros ( np . s i z e ( f l a t _ a r g s ) )
f o r
i
in
range (T −1 ,
...
c u r r e n t
time .
v j p _ c u r _ t = np . dot ( func ( yt [ i ,
: ] ,
t [ i ] , ∗func_args ) ,
g [ i ,
: ] )
t i m e _ v j p _ l i s t . append ( v j p _ c u r _ t )
vjp_t0 = vjp_t0 −v j p _ c u r _ t
# Run augmented
system
backwards
to
...
aug_y0 = np . hstack ( ( yt [ i ,
: ] ,
vjp_y ,
vjp_t0 ,
v j p _ a r g s ) )
aug_ans = o d e i n t ( augmented_dynamics ,
aug_y0 ,
np . a r r a y ( [ t [ i ] ,
t [ i −1 ] ] ) ,
t u p l e ( ( f l a t _ a r g s , ) ) ,
∗∗kwargs )
_ ,
vjp_y ,
vjp_t0 ,
v j p _ a r g s = unpack ( aug_ans [ 1 ] )
# Add g r a d i e n t
from
c u r r e n t
ou tput .
vjp_y = vjp_y + g [ i −1 ,
: ]
t i m e _ v j p _ l i s t . append ( vjp _t0 )
vjp_times = np . hstack ( t i m e _ v j p _ l i s t ) [: : −1 ]
r e t u r n
None ,
vjp_y ,
vjp_times ,
u n f l a t t e n ( v j p _ a r g s )
r e t u r n
v j p _ a l l
17
def
grad_argnums_wrapper ( a l l _ v j p _ b u i l d e r ) :
# A g e n e r i c
autograd
h e l p e r
f u n c t i o n .
...
vjps .
def
b u i l d _ s e l e c t e d _ v j p s ( argnums ,
ans ,
combined_args ,
kwargs ) :
vjp_func = a l l _ v j p _ b u i l d e r ( ans ,
∗combined_args ,
∗∗kwargs )
def
chosen_vjps ( g ) :
...
chosen_vjps
r e t u r n
b u i l d _ s e l e c t e d _ v j p s
defvjp_argnums ( odeint ,
grad_argnums_wrapper ( g r a d _ o d e i n t _ a l l ) )
Appendix E
...
q(zt0|{xti, ti}i, φ) = N(zt0|µzt0 , σz0),
(53)
